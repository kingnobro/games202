...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      ...
      auto L = normalize(sampleList.directions[i]);
      auto H = normalize(L + V);
      float pdf = sampleList.PDFs[i];
      float NdotL = dot(L, N);

      float F = R0 + (1.0 - R0) * pow(1.0f - dot(V, H), 5.0f);
      float D = DistributionGGX(N, H, roughness);
      float G = GeometrySmith(roughness, dot(L, H), dot(V, H));
      float fr = F * G * D / (4.0f * NdotL * NdotV);
