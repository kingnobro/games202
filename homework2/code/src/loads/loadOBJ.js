						...
						if (child.isMesh) {
							...
							switch (objMaterial) {
								...
								case 'PRTMaterial':
									precomputeL_mat = [[], [], []];
									for (let i = 0; i < 9; i++) {
										precomputeL_mat[0].push(precomputeL[0][i][0]);
										precomputeL_mat[1].push(precomputeL[0][i][1]);
										precomputeL_mat[2].push(precomputeL[0][i][2]);
									}
									console.log('hello', precomputeL_mat);
									material = buildPRTMaterial(precomputeL_mat, "./src/shaders/PRTShader/PRTVertex.glsl", "./src/shaders/PRTShader/PRTFragment.glsl");
									break;
								case 'SkyBoxMaterial':
