...
void main() {
  ...
  for (int i = 0; i < SAMPLE_NUM; i++) {
    ...
    vec3 dir = normalize(localDir.x * b1 + localDir.y * b2 + localDir.z * n);
    if (RayMarch(vPosWorld.xyz, dir, hitPos)) {
      vec2 uv1 = GetScreenCoordinate(hitPos);
      vec3 diffuse_1 = EvalDiffuse(wi, dir, uv1);
      vec3 Le_1 = EvalDirectionalLight(uv1);
      L_indirect += EvalDiffuse(dir, wo, uv) / pdf * diffuse_1 * Le_1;
    }
  }
